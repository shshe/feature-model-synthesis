...
class DNFAdapterTest extends FunSuite with ShouldMatchers {
  ...
  test("More OR-Groups") {
    ...
    DNFAdapter.orGroups(dnf,1) should be (Set(Set(2,3)))
    DNFAdapter.orGroups(dnf,2) should be (Set())
    DNFAdapter.orGroups(dnf,3) should be (Set(Set(1,4,5)))
    DNFAdapter.orGroups(dnf,4) should be (Set())
    DNFAdapter.orGroups(dnf,5) should be (Set())
  }

  test("More OR-Groups (2)") {
    val dnf: DNF = Set(Set(3,4), Set(3,5)).negateUnboundedVars(5)
    DNFAdapter.orGroups(dnf,1) should be (Set())
    DNFAdapter.orGroups(dnf,2) should be (Set())
    DNFAdapter.orGroups(dnf,3) should be (Set(Set(4,5)))
    DNFAdapter.orGroups(dnf,4) should be (Set())
    DNFAdapter.orGroups(dnf,5) should be (Set())
  }
  ...
  test("OR-Groups with hierarchy") {
    ...
    DNFAdapter.orGroups(dnf,1, Set(2,3)) should be (Set(Set(2,3)))
    DNFAdapter.orGroups(dnf,2, Set()) should be (Set())
    DNFAdapter.orGroups(dnf,3, Set(4,5)) should be (Set(Set(4,5)))
    DNFAdapter.orGroups(dnf,4, Set()) should be (Set())
    DNFAdapter.orGroups(dnf,5, Set()) should be (Set())
  }
  ...
  test("OR-Groups with hierarchy (2)") {
    ...
    DNFAdapter.orGroups(dnf,1, Set(2,3,4)) should be (Set(Set(3,4), Set(2,3)))
  }

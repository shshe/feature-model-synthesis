...
class ImplBuilderTest extends FunSuite {
  ...
  test("implication is found") {
    val b = new SATBuilder(List(List(-1,2)), 2) with ImplBuilder
  ...
  test("transitive implications are found") {
    val b = new SATBuilder(List(List(-1,2), List(-2,1), List(-2,3)), 3) with ImplBuilder
    assert(b.implication(1,2))
  ...
  test("mkImplicationGraph - implications are found") {
    val b = new SATBuilder(List(List(-1,2), List(-2,3), List(-3,1)),3) with ImplBuilder
